...
async function ensureSchemaOnce() {
  ...
  try {
    const r = await exec("SELECT v FROM tracking_lib WHERE k='schemaVer:adminusers' LIMIT 1");
  ...
  await ensureTables();
  try {
    await exec("INSERT INTO tracking_lib (k, v, updated_at) VALUES ('schemaVer:adminusers', " + q(JSON.stringify(SCHEMA_VER)) + ", now()) ON CONFLICT (k) DO UPDATE SET v=EXCLUDED.v, updated_at=now()");
  } catch (e) { }
  __schemaOK = true;
}
