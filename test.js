...
test('单张高普8K彩色系列 11色 × 7档（2026-08-24 用户规则：高普基础4 + 颜色价[含厚度加价]，1000mm ×1.25）', () => {
  const g = (surface, t, w) => { const r = PricingEngine.getSurfaceFee(surface, t, w, '304'); return r ? (r.sqmPrice != null ? r.sqmPrice : r.price) : null; };
  // 黄钛金 窄板 7 档（0.24-1.20/1.21-1.49/1.50-1.59/1.60-1.69/1.70-1.79/1.80-1.89/1.90-2.00）
  eq([0.5, 1.3, 1.55, 1.65, 1.75, 1.85, 1.95].map(t => g('单张高普8K黄钛金', t, 1219)).join('/'), '10/12/14/24/27/30/34', '黄钛金窄板7档（1.21-1.49=12 用户例）');
  eq(g('单张高普8K黄钛金', 0.5, 1000), 12.5, '黄钛金1000 0.5→12.5');
  eq(g('单张高普8K黄钛金', 1.3, 1000), 15, '黄钛金1000 1.3→15');
  eq(g('单张高普8K黄钛金', 1.95, 1000), 42.5, '黄钛金1000 1.95→42.5');
  ...
  eq(g('单张高普8K宝石蓝', 1.3, 1219), 13.5, '宝石蓝窄板1.3→13.5');
  eq(g('单张高普8K宝石蓝', 1.3, 1000), 16.88, '宝石蓝1000 1.3→16.88');
  ...
  eq(g('单张高普8K中国红', 1.95, 1219), 44.5, '中国红窄板1.95→44.5');
  eq(g('单张高普8K黄钛金', 2.1, 1219), null, '彩色>2.00无加价→null');
  eq(g('单张高普8K黄钛金', 0.5, 1500), null, '彩色1500宽无价→null');
  // 单张模式完整计算：黄钛金 1.3mm 平板 1219×2500，表面加工费 = 12元/㎡ × 3.0475㎡ = 36.57
  ...
  eq(r.success && r.detail.sheetSurfaceCost, 36.57, '表面加工费36.57 实际=' + (r.success ? r.detail.sheetSurfaceCost : JSON.stringify(r.errors)));
  // 别名
